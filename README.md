...
![Version](https://img.shields.io/badge/Version-1.3.2-00E5FF?style=flat)

---

## 📋 Overview | نمای کلی

NetFix Protocol monitors Wi-Fi/LAN connectivity by pinging domestic and international hosts, shows a live status/chart, and can repair Wi-Fi outages automatically. Version **1.3.2** keeps the full v1.3.1 feature set and adds the Auto Updater flow for GitHub Releases checks, update badge status, and click-to-update EXE replacement.

NetFix Protocol اتصال Wi-Fi یا LAN را با پینگ گرفتن از هاست‌های داخلی و خارجی بررسی می‌کند، وضعیت را زنده نمایش می‌دهد و در صورت قطعی کامل می‌تواند اتصال Wi-Fi را تعمیر کند. در نسخه **1.3.2** علاوه بر امکانات نسخه 1.3.1، Auto Updater برای بررسی GitHub Releases، نمایش وضعیت آپدیت و دریافت/اجرای EXE جدید اضافه شده است.
...
```text
NetFix_PROTOCOL_v1.3.2_Release\NetFix_PROTOCOL_1.3.2.exe
```
...
The project includes a PyInstaller spec for 1.3.2:

```bash
python -m PyInstaller --clean --noconfirm --distpath "C:\Users\User\Desktop\NetFixProtocol\NetFix_PROTOCOL_v1.3.2_Release" --workpath "build\NetFix_PROTOCOL_1.3.2" NetFix_PROTOCOL_1.3.2.spec
```
...
NetFixProtocol/
    |
    `-- NetFix_PROTOCOL_v1.3.2/  # Current 1.3.2 source folder prepared for GitHub
        |
        ...
        |-- Net-fix-updater.py   # GitHub Releases checker, update badge, EXE download/restart helper
        ...
        |-- NetFix_PROTOCOL_1.3.2.spec
        |                         # PyInstaller build config for the 1.3.2 EXE
...
| `Net-fix-updater.py` | Checks GitHub Releases on startup, updates the menu-side badge, downloads/runs newer EXE updates |
...
1. Open `NetFix_PROTOCOL_1.3.2.exe` or run `python main.py`.
...
### 1.3.2 (Latest) — Update Log from 1.3.1

- **Auto Updater**: Added `Net-fix-updater.py` to check GitHub Releases on startup, show the update badge, download the latest EXE when `Please update` is clicked, run the new app, and remove the old EXE after the new version starts.


### 1.3.1 — Update Log from v1.3

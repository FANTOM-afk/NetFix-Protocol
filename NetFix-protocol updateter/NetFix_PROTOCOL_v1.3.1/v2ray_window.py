...
def open_v2ray(
    ...
    main.grid_columnconfigure(0, weight=3, minsize=380)
    ...
    ctk.CTkLabel(
        main,
        text="> V2RAY PROFILES",
        font=ctk.CTkFont(family=colors["FONT"], size=16, weight="bold"),
    ...
    status_label = ctk.CTkLabel(
        main,
        text=status_text,
        font=ctk.CTkFont(family=colors["FONT"], size=12, weight="bold"),
    ...
    port_entry = ctk.CTkEntry(
        ...
        font=ctk.CTkFont(family=colors["FONT"], size=13),
    )
    ...
    bg_check = ctk.CTkCheckBox(
        ...
        font=ctk.CTkFont(family=colors["FONT"], size=11, weight="bold"),
    ...
    left.grid(row=4, column=0, sticky="nsew", padx=(18, 8), pady=(0, 8))
    left.grid_columnconfigure(0, weight=1)
    left.grid_rowconfigure(1, weight=1)

    right = ctk.CTkFrame(main, fg_color="transparent")
    right.grid(row=4, column=1, sticky="nsew", padx=(8, 18), pady=(0, 8))
    ...
    selected_specs = ctk.CTkLabel(
        ...
        font=ctk.CTkFont(family=colors["FONT"], size=12),
    )
    ...
    for idx, (key, label) in enumerate(detail_items):
        ...
        value_label = ctk.CTkLabel(
            ...
            text="-",
            text_color=colors["TEXT"],
            font=ctk.CTkFont(family=colors["FONT"], size=11),
        )
    ...
    raw_box.grid(row=6, column=0, sticky="nsew", pady=(0, 6))
    ...
    def clear_raw_placeholder(_event=None) -> None:
        ...
        if raw_placeholder_visible:
            ...
            for key, label in detail_labels.items():
                label.configure(text_color=colors["DIM_TEXT"] if key == "ping" else colors["TEXT"])
    ...
    def fill_form(profile: Optional[Dict]) -> None:
        ...
        for key, label in detail_labels.items():
            if key == "ping":
                ...
                label.configure(text_color=colors["PING_GREEN"] if value != "--" else colors["DIM_TEXT"])
            else:
    ...
    def refresh_groups() -> None:
        ...
        for col, group in enumerate(values):
            ...
            btn = ctk.CTkButton(
                ...
                hover_color=colors["FRAME_BORDER"],
                font=ctk.CTkFont(family=colors["FONT"], size=11, weight="bold" if active else "normal"),
            )
    ...
    def refresh_profiles() -> None:
        ...
        for row, profile in enumerate(visible_profiles):
            ...
            ping_label = ctk.CTkLabel(
                ...
                text=ping_value,
                width=70,
                text_color=colors["PING_GREEN"] if ping_value != "--" else colors["DIM_TEXT"],
                font=ctk.CTkFont(family=colors["FONT"], size=12, weight="bold"),
            )
            ...
            copy_btn = ctk.CTkButton(
                ...
                hover_color=colors["FRAME_BORDER"],
                font=ctk.CTkFont(family=colors["FONT"], size=10, weight="bold"),
            )
    ...
    def open_new_group_window() -> None:
        ...
        ctk.CTkLabel(
            frame,
            text="NEW GROUP",
            font=ctk.CTkFont(family=colors["FONT"], size=14, weight="bold"),
            text_color=colors["ACCENT_CYAN"],
    ...
    def refresh_group_clicked() -> None:
        group = group_var.get() or "Default"
        url = v2ray_profiles.group_subscription_url(store, group)
        if not url:
            ui_message("[!] Current group has no subscription URL.", colors["ACCENT_RED"])
            open_add_subscription_window()
            return
        ui_message(f"[*] Refreshing group: {group}", colors["ACCENT_CYAN"])
        threading.Thread(target=import_subscription_worker, args=(url, group), daemon=True).start()
    ...
    refresh_group_btn = make_action_btn("REFRESH GROUP", action_command(refresh_group_clicked), colors["ACCENT_CYAN"], 0, 2)
